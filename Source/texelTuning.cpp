...
template<typename T> struct TexelParam {
  public:
   TexelParam(
       T&                                   _accessor,
       const T&                             _inf,
...
template<typename T> std::ostream& operator<<(std::ostream& os, const TexelParam<T>& p) {
   const T& t = p;
   os << t;
   return os;
}
...
void Randomize(std::vector<Texel::TexelInput>& data) { std::shuffle(data.begin(), data.end(), std::default_random_engine(0)); }
...
std::vector<double> ComputeGradient(std::vector<TexelParam<ScoreType>>& x0,
...
std::vector<TexelParam<ScoreType>> TexelOptimizeGD(const std::vector<TexelParam<ScoreType>>& initialGuess,
                                                   std::vector<Texel::TexelInput>&           data,
...
std::vector<TexelParam<ScoreType>> TexelOptimizeNaive(const std::vector<TexelParam<ScoreType>>& initialGuess,
                                                      std::vector<Texel::TexelInput>&           data,
                                                      const size_t                              batchSize) {
   DynamicConfig::disableTT = true;
...
*/
   ...
   for (auto loops = 0; loops < 1000; ++loops) {
      ...
      for (auto it = todo.begin(); it != todo.end(); ++it) {
         ...
         std::vector<Texel::TexelParam<ScoreType>> optim =
             Texel::TexelOptimizeGD(guess[*it], data, batchSize, std::max(64ul, guess[*it].size()), *it);
