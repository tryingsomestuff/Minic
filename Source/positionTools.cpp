...
std::string GetFENShort2(const Position &p) { // "rnbqkbnr/ppp1pppp/8/3p4/4P3/8/PPPP1PPP/RNBQKBNR w KQkq d5
    ...
    if ( DynamicConfig::FRC ){
       if ( p.castling & C_wqs ) { for( Square s = Sq_a1 ; s <= Sq_h1 ; ++s ){ if ( s < p.king[Co_White] && p.board_const(s)==P_wr ) { castlingChar[1] = std::toupper(FileNames[SQFILE(s)][0]); break; } } }
       if ( p.castling & C_wks ) { for( Square s = Sq_a1 ; s <= Sq_h1 ; ++s ){ if ( s > p.king[Co_White] && p.board_const(s)==P_wr ) { castlingChar[0] = std::toupper(FileNames[SQFILE(s)][0]); break; } } }
       if ( p.castling & C_bqs ) { for( Square s = Sq_a8 ; s <= Sq_h8 ; ++s ){ if ( s < p.king[Co_Black] && p.board_const(s)==P_br ) { castlingChar[3] = std::tolower(FileNames[SQFILE(s)][0]); break; } } }
       if ( p.castling & C_bks ) { for( Square s = Sq_a8 ; s <= Sq_h8 ; ++s ){ if ( s > p.king[Co_Black] && p.board_const(s)==P_br ) { castlingChar[2] = std::tolower(FileNames[SQFILE(s)][0]); break; } } }
    }

    if (p.castling & C_wks) { ss << castlingChar[0]; withCastling = true; }
    if (p.castling & C_wqs) { ss << castlingChar[1]; withCastling = true; }
    if (p.castling & C_bks) { ss << castlingChar[2]; withCastling = true; }
    if (p.castling & C_bqs) { ss << castlingChar[3]; withCastling = true; }
    if (!withCastling) ss << "-";
