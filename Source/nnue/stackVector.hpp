...
struct StackVector {
   ...
   FORCE_FINLINE StackVector<T, dim, Q>& add_(const T2* other) {
...
#pragma omp simd
   ...
   template<typename T2> 
   FORCE_FINLINE StackVector<T, dim, Q>& sub_(const T2* other) {
#ifdef USE_SIMD_INTRIN
      if constexpr (std::is_same_v<T, int16_t> && std::is_same_v<T2, int16_t>) {
...
#pragma omp simd
   ...
   FORCE_FINLINE StackVector<T, dim, Q>& fma_(const T2 c, const T3* other) {
#pragma omp simd
   ...
   template<typename F> 
   FORCE_FINLINE StackVector<T, dim, Q>& apply_(F&& f) {
#pragma omp simd
   ...
   FORCE_FINLINE StackVector<T, dim, Q>& activation_() { simdActivation<dim,Q>(data); return *this;}
#endif
...
#pragma omp simd
  ...
  template <typename U> 
   [[nodiscard]] FORCE_FINLINE StackVector<U, dim, Q> dequantize(const U& scale) const {
...
FORCE_FINLINE StackVector<T, dim0 + dim1, Q> splice(const StackVector<T, dim0, Q>& a, const StackVector<T, dim1, Q>& b) {
