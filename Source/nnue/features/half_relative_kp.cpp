...
inline IndexType HalfRelativeKP<AssociatedKing>::MakeIndex(
  ...
  const IndexType p = IndexType(orient(perspective, s) + kpp_board_index[PieceIdx(pc)][perspective]);
...
inline IndexType HalfRelativeKP<AssociatedKing>::MakeIndex(
  ...
  const IndexType relative_file = SQFILE(sq_p) - SQFILE(sq_k) + (W / 2);
  const IndexType relative_rank = SQRANK(sq_p) - SQRANK(sq_k) + (H / 2);
