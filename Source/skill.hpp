...
namespace Skill {
    ...
    [[nodiscard]] inline DepthType limitedDepth() { return DepthType(1 + 2 * std::sqrt(std::max(0,int(DynamicConfig::level-20)))); } ///@todo to be tuned
    [[nodiscard]] inline uint64_t limitedNodes() { return uint64_t(std::exp((double)(convertLevel2Elo()-400)/140)); } ///@todo to be tuned
