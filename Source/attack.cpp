...
constexpr array1d<BitBoard,NbSquare> bishopMagics = {
...
constexpr array1d<BitBoard,NbSquare> rookMagics = {
...
bool isAttacked(const Position &p, BitBoard bb) { // copy ///@todo should be done without iterating over Square !
   while (bb)
      if (isAttacked(p, BB::popBit(bb))) return true;
   return false;
}
