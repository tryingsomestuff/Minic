...
def do_plots(out_filename, root_dirs, elo_range, loss_range, split):
    ...
    ax_train_loss = fig.add_subplot(411)
    ax_val_loss = fig.add_subplot(412)
    ax_elo = None
    ...
    for user_root_dir in root_dirs:
        ...
        if ordo_file:
            ...
            ax_eloloss = fig.add_subplot(414)
            ax_eloloss.set_xlabel('loss')
            ax_eloloss.set_ylabel('Elo')
            loss = loss[:len(elos)]
            ax_eloloss.errorbar(loss,elos, yerr=errors, fmt='.', color='gray', marker=None, capsize=2)
