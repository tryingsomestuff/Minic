...
import shutil, subprocess, requests, zipfile, os, math, json, stat
...
# path to my own cutechess
#CUTECHESS = '/ssd/cutechess/projects/cli/cutechess-cli'
CUTECHESS = './cutechess'
...
def getEngine(data):
    ...
    st = os.stat('tmp/{0}/'.format(unzipname) + 'tools/build.sh')
    os.chmod('tmp/{0}/'.format(unzipname) + 'tools/build.sh', st.st_mode | stat.S_IEXEC)
    subprocess.Popen(
        ['./tools/build.sh'],
...
def getCutechessCommand(data, scalefactor):
    ...
    devflags = (
        ...
        ' arg=-xboard arg=-ttSizeMb arg=256 arg=-threads arg=1' +
...
def singleCoreBench(name, outqueue):
    ...
    try:
        ...
        output, data = subprocess.Popen(
            '{0} bench -ttSizeMb 256'.format(dir).split(),
...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            print('Machine id : ' + str(postdata['machineid']))
